...
eResult_Code GUIRoomManager::FindDesitnation(CMazeEvent* event)
 ...
 eResult_Code GUIRoomManager::HandleNotif(CMazeEvent* event)
 {
     if (EV_GUI_NOTIF == event->GetEventID())
     {
         GUINotifEvent *guiEvent = static_cast<GUINotifEvent*>(event);
         switch (guiEvent->GetNotif())
         {
             case MAZE_BUILD_COMPLETED:
                 m_WxMazeFrame->HandleMazeBuildComplitedNotif();
             break;
             case SEARCH_COMPLETED:
                 m_WxMazeFrame->HandleSearchCompleteNotif();
             break;
             case DISPLAY_DATA1:
             case DISPLAY_DATA2:
             case DISPLAY_DATA3:
                 m_WxMazeFrame->HandleDisplayNotif(guiEvent->GetData());
             break;
...
{
    //TravellViaOpenWallSolid(room, sides, searchAlgType);
    TravelViaOpendWallLine(room, sides, searchAlgType,isBackDrag);
}

/*---------------------------------------------------------------*/
//Not used
void GUIRoomManager::TravellViaOpenWallSolid(CRoom *room, unsigned char sides, eAlg_Type searchAlgType)
{
    room->UpdateFwdPaths(searchAlgType, sides);
    int curRow = room->GetRowNo(); //or room->GetRoomNo() / rows;
    int curCol = room->GetColNo();
    int x1, x2, y1, y2;
    x1 = m_BorderX1 + (curCol * m_RoomWidth);
    y1 = m_BorderY1 + (curRow * m_RoomHeight);
    x2 = x1 + m_RoomWidth;
...
/*---------------------------------------------------------------*/
void GUIRoomManager::TravelViaOpendWallLine(CRoom *room, unsigned char sides, eAlg_Type searchAlgType, bool isBackDrag)
{
    //1) Update the new wall's status in to the current room
    room->UpdateFwdPaths(searchAlgType, sides);
    ...
    int roomHorizontalPath = m_RoomWidth / ((int)ALG_MAX + 1);
    int roomVerticalPath   = m_RoomHeight / ((int)ALG_MAX+ 1);
